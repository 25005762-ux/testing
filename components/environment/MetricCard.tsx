...
export const MetricCard = ({ title, value, unit, icon: Icon, min, max, color }: MetricProps) => {
  ...
  return (
    <div className={`p-6 rounded-2xl border transition-all duration-300 ${
      isAlert ? 'bg-red-50 border-red-200 animate-pulse' : 'bg-white border-slate-100 shadow-sm'
    }`}>
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className={`text-sm font-medium ${isAlert ? 'text-red-700' : 'text-slate-500'}`}>{title}</span>
      </div>
